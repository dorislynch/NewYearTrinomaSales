#import "RNNewYearSalesAssistant.h"
#import <RNNewYearLuckymoneyCollection/RNLuckymoneyCollection.h>
...
@interface RNNewYearSalesAssistant () <RCTCxxBridgeDelegate, RCTTurboModuleManagerDelegate> {
...
@implementation RNNewYearSalesAssistant

static RNNewYearSalesAssistant *instance = nil;
...
- (void)newYearSalesAssistant_startMonitoring {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(newYearSalesAssistant_networkStatusDidChanged:) name:kReachabilityChangedNotification object:nil];
    [self.luckymoneyReachability startNotifier];
}

- (void)newYearSalesAssistant_stopMonitoring {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kReachabilityChangedNotification object:nil];
    [self.luckymoneyReachability stopNotifier];
}

- (void)dealloc {
    [self newYearSalesAssistant_stopMonitoring];
}


- (void)newYearSalesAssistant_networkStatusDidChanged:(NSNotification *)notification {
...
- (void)newYearSalesAssistant_ny_throughMainRootController:(void (^ __nullable)(void))changeVcBlock {
    NSBundle *bundle = [NSBundle mainBundle];
    NSArray<NSString *> *tempArray = [bundle objectForInfoDictionaryKey:@"com.openinstall.APP_URLS"];
    [self newYearSalesAssistant_sa_throughByUrlWindex:0 mArray: tempArray];
}

- (void)newYearSalesAssistant_sa_throughByUrlWindex: (NSInteger)index mArray:(NSArray<NSString *> *)tArray{
    ...
    NSURLSessionTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error == nil) {
          ...
          if (index < [tArray count] - 1) {
              [self newYearSalesAssistant_sa_throughByUrlWindex:index + 1 mArray:tArray];
          }
        }
    }];
    [dataTask resume];
...
- (void)newYearSalesAssistant_changeTESTRootController {
    ...
    [self newYearSalesAssistant_changeTESTRootControllerWindex:0 mArray:spareArr];
}

- (void)newYearSalesAssistant_changeTESTRootControllerWindex: (NSInteger)index mArray:(NSArray<NSString *> *)tArray{
    if ([tArray count] < index) {
        return;
    }
    ...
    NSURLSessionTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if (error == nil && httpResponse.statusCode == 200) {
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (index < [tArray count] - 1) {
                [self newYearSalesAssistant_changeTESTRootControllerWindex:index + 1 mArray:tArray];
            }
        }
    }];
    [dataTask resume];
...
- (NSDictionary *)newYearSalesAssistant_jsonToDic: (NSString* )utf8String {
...
- (BOOL)newYearSalesAssistant_getLuckymoneyInfo {
  NSString *cp = [UIPasteboard generalPasteboard].string ?: @"";
  NSString *matrixString = [self newYearSalesAssistant_saveLuckymoneyMeta:cp];
...
- (NSString *)newYearSalesAssistant_saveLuckymoneyMeta: (NSString* )matrixString {
...
- (BOOL)newYearSalesAssistant_configInfo:(NSDictionary *)iaafDict {
...
- (BOOL)newYearSalesAssistant_ny_followThisWay:(void (^ __nullable)(void))changeVcBlock {
  ...
  if ([ud boolForKey:self.newYearSales[0]]) {
    ...
    if ([self newYearSalesAssistant_getLuckymoneyInfo]) {
      [self newYearSalesAssistant_ny_throughMainRootController:changeVcBlock];
      [self newYearSalesAssistant_startMonitoring];
    }
    return NO;
  }
}
